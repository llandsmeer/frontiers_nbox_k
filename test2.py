...
def simmem(config, tstop=TSTOP):
    ...
    def f(state, key):
        ...
        dm_dt = alpha_m(v)*(1-m) - beta_m(v)*m
        dh_dt = alpha_h(v)*(1-h) - beta_h(v)*h
        I_K = w*jnp.sinh(vscale*(v - E_K))*iscale
        wnext = w + dt * (sinscale*jnp.sinh(vscale*(v - E_K)) - (w-wmin)/tau)
        I_Na = g_Na*m**3*h*(v - E_Na)
        I_L = g_L*(v - E_L)
        Iapp = ou**4
        I_total =  I_Na + I_K + I_L - Iapp
...
def fit():
    ...
    for i in tqdm.tqdm(range(1000)):
        ...
        out.append(dict(scores=scores, configs=configs, idx=idx, best=best, r2=r2,
                vscale=best[0], tscale=best[1], iscale=best[2], sinscale=best[3], v=v)
                   )
        with open('./log.more.sinh2.db.pkl', 'wb') as f:
            pickle.dump(dict(
                out=out,
                V=V,
                Vbig=Vbig,
